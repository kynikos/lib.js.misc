...
module.exports = (img, mag, frameW, frameC, minMarg, scrBar) ->
    # Controlla che l'elemento esista e sia un <img>
    if img and img.tagName == 'IMG'
        ...
        rappLatiWin = window.innerWidth / window.innerHeight
        winWidth = window.innerWidth
        winHeight = window.innerHeight
        rappLati = zimg.width / zimg.height

        # Ridimensionamento basato sulla larghezza
        if rappLati >= rappLatiWin and (zimg.width + 2 * (frameW + minMarg) + scrBar) > winWidth
            zimg.width = winWidth - 2 * (frameW + minMarg) - scrBar
            zimg.height = zimg.width / rappLati

        # Ridimensionamento basato sull'altezza
        else if rappLati < rappLatiWin and (zimg.height + 2 * (frameW + minMarg)) > winHeight
        ...
        zimg.style.border = frameW + 'px solid ' + frameC

        # Onclick per la chiusura
        zimg.onclick = new Function('this.parentNode.removeChild(this); return false;')
        zimg.style.cursor = 'pointer'

        # Rimuovi l'eventuale immagine zoomata precedente
